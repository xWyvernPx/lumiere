...
const InnerButtons = ({ showNotice }: SocialLoginButtonsProps) => {
  ...
  const handleSocialSuccess = async (provider: string, token: string) => {
    try {
      const response = await socialLoginMutation.mutateAsync({ provider, token });
    ...
    <div className="space-y-6 mt-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <FacebookLogin
          ...
          callback={(response: any) => {
            if (response.accessToken) {
              handleSocialSuccess('facebook', response.accessToken);
            } else {
        ...
        <AppleSignin
          ...
          uiType="dark"
          onSuccess={(response: any) => handleSocialSuccess('apple', response.authorization.id_token)}
